import { ArrowRight, Camera, Award, FileCheck, Target, Heart, Zap } from 'lucide-react';
...
export function CoachBio() {
  ...
  // Core values untuk ditampilkan di modal
  const values = [
      ...
      <Dialog open={isBioOpen} onOpenChange={setIsBioOpen}>
        <DialogContent className="bg-zinc-900 border-white/10 text-white max-w-3xl max-h-[85vh] overflow-y-auto custom-scrollbar shadow-2xl">
          ...
          <div className="space-y-8 py-6">
            ...
            {/* Core Values Section */}
            <div>
              <h4 className="text-xs font-black uppercase tracking-widest text-white/30 mb-4">Core Values</h4>
              <div className="grid sm:grid-cols-2 gap-4">
                {values.map((value, index) => {
                  const Icon = value.icon;
                  return (
                    <div key={index} className="bg-black/40 border border-white/5 p-4 rounded-xl flex items-start gap-4 hover:border-orange-500/30 transition-colors">
                      <div className="bg-orange-500/10 p-3 rounded-lg flex-shrink-0">
                        <Icon className="w-5 h-5 text-orange-500" />
                      </div>
                      <div>
                        <Editable id={value.titleId} as="h4" className="text-white font-bold text-sm mb-1" defaultText={value.defaultTitle} />
                        <Editable id={value.descId} as="p" className="text-white/50 text-xs leading-relaxed" defaultText={value.defaultDesc} />
                      </div>
                    </div>
                  );
                })}
